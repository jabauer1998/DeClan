...
import declan.backend.arm.descriptor.ArmAddressDescriptor;
import declan.backend.arm.descriptor.ArmDescriptorState;
import declan.backend.arm.descriptor.ArmRegisterDescriptor;
import declan.backend.arm.descriptor.ArmRegisterElement;
            ...
            for(BlockNode block : flowGraph.getBlocks()){
                ...
                for(FlowGraphNode node : block.getPredecessors()){
                        if(!node.getICode().isEmpty()) {
                                ArmDescriptorState outputOfPredecessor = super.getOutputSet(node.getICode().get(node.getICode().size() - 1));
