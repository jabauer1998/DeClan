...
import declan.frontend.ast.DeclarationVisitor;
...
import declan.frontend.builder.LibraryBuilder;
import declan.frontend.builder.ProgramBuilder;
import declan.frontend.builder.StatementBuilder;
import declan.frontend.builder.SymbolBuilder;
import declan.frontend.builder.SymbolBuilder.SymbolBuilderSearchStrategy;
...
import declan.utils.symboltable.entry.IdentEntryList;
import declan.utils.symboltable.entry.ProcedureEntry;
import declan.utils.symboltable.entry.StringEntry;
...
import static declan.frontend.IrRegisterGenerator.*;
...
public class MyICodeGenerator{
  ...
  public MyICodeGenerator(Config cfg, ErrorLog errorLog) {
    ...
    if(this.cfg != null)
        if(this.cfg.containsFlag("debug"))
                Utils.createFile("./tmp/icode.txt");
  }
  ...
  public Lib generateLibraryIr(Library lib){
    ...
    if(this.cfg != null)
        if(this.cfg.containsFlag("debug"))
                Utils.writeToFile("./tmp/icode.txt", library.toString());
    return library;
  }
  ...
  public Prog generateProgramIr(Program program) {
    ...
    if(this.cfg != null)
        if(this.cfg.containsFlag("debug"))
                Utils.writeToFile("./tmp/icode.txt", prog.toString());
    return prog;
  }
